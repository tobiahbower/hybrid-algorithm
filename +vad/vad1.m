...
function vadResult = vad1(x, fs, len)
    ...
    for i=1:nFrames
		frame = frames(:,i);
		e(i) = vad.energy(frame.*window);
        zcr(i) = vad.zeroCrossRate(frame);
    end
    ...
    avgSilenceEnergy = vad.energy(x(1+nIgnoredSamples:nIgnoredSamples+nSilenceSamples).*window);
	...
	silencezeroCrossRate = vad.zeroCrossRate(x(1+nIgnoredSamples:nIgnoredSamples+nSilenceSamples));
    peakEnergy = max(e);
    ...
    for i=1:nPoints
		...
		for j=searchWindowIni:searchWindowFin
			zeroCrossRateVicinity(j-searchWindowIni+1) = vad.zeroCrossRate(frames(:,j));
        end
	...
	for i=1:nUtterances
		...
		for j=endPoints(i,1):endPoints(i,2)
            ...
            while(~isempty(stack))
				...
				frameEnergy = vad.energy(frame.*window(1:fin-ini+1));
				framezeroCrossRate = vad.zeroCrossRate(frame);
